...
def main_readCartHeader():
    ...
    app.ROMtitleLabel.set("ROM Title: " + str(Header[0x34:0x43], 'utf-8', 'ignore'))
...
def main_LoadROM():
    ...
    global ROMbuffer
    root.update()
    ROMfileName = askopenfilename(
        filetypes=(("GB ROM File", "*.GB"), ("GBC ROM File", "*.GBC"), ("GBA ROM File", "*.GBA"), ("All Files", "*.*")))
    if ROMfileName:
...
def main_SaveRAM():
    ...
    global RAMbuffer
    print(len(RAMbuffer))
    RAMfileName = asksaveasfilename(defaultextension=".SAV",
                                    filetypes=(("GB/C/A SRAM File", "*.SAV"), ("All Files", "*.*")))
    if RAMfileName:
...
def mx1():
    dev.write(0x01,
              [0x0A, 0x00, 0x07, 0x3f, 0x00, 0x41, 0x55, 0x55, 0xAA, 0x2A, 0xAA, 0x55, 0x55, 0x55, 0x80, 0x55, 0x55,
               0xAA, 0x2A, 0xAA, 0x55, 0x55, 0x55, 0x10])
    print("Erasing Flash")
    while main_JPN_Read(0) != 255:
        pass
    print("Chip erase complete")
...
def mx2():
    dev.write(0x01,
              [0x23, 0x00, 0x00, 0x20, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55,
...
def main_JPN_test():
    ...
    for k in range(16):
        dev.write(0x01,
                  [0x0A, 0x00, 0x10, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55,
...
def main_JPN_F1(Address, Data):
    ...
    AddLo = Address & 0xFF
    dev.write(0x01,
              [0x0A, 0x00, 0x05, 0x01, 0x20, 0x0F, 0x01, 0x25, AddHi, 0x01, 0x26, AddLo, 0x01, 0x27, Data, 0x01, 0x3F,
               0xA5])
    USBbuffer = dev.read(0x81, 64)
...
def main_JPN_F4():
    dev.write(0x01,
              [0x0A, 0x00, 0x09, 0x01, 0x20, 0x0A, 0x01, 0x25, 0x62, 0x01, 0x26, 0x04, 0x01, 0x27, 0x00, 0x01, 0x3F,
               0xA5, 0x01, 0x20, 0x01, 0x01, 0x3F, 0xA5, 0x01, 0x20, 0x02, 0x01, 0x3F, 0xA5])
    USBbuffer = dev.read(0x81, 64)
...
def main_GBA_GetByte(Address):
    ...
    ROMbuffer = dev.read(0x81, 64)
    return ROMbuffer[0], ROMbuffer[1], ROMbuffer[2], ROMbuffer[3], ROMbuffer[4], ROMbuffer[5], ROMbuffer[6], ROMbuffer[
        7]
...
def main_GBA_Dump():
    ROMfileName = asksaveasfilename(defaultextension=".GBA",
...
def main_GBA_Sector_Erase(Sector):
    ...
    dev.write(0x01,
              [0x31, 0x06, 0x00, 0x05, 0x55, 0x00, 0xA9, 0x00, 0x02, 0xAA, 0x00, 0x56, 0x00, 0x05, 0x55, 0x00, 0x80,
               0x00, 0x05, 0x55, 0x00, 0xA9, 0x00, 0x02, 0xAA, 0x00, 0x56, Shi, Slo, 0x00, 0x00, 0x30, ])
    ROMbuffer = dev.read(0x81, 64)
    # Is flash busy?
    dev.write(0x01, [0x33])
...
def main_GBA_Dump64kSRAM():
    RAMfileName = asksaveasfilename(defaultextension=".SAV",
                                    filetypes=(("GBA Save File", "*.SAV"), ("All Files", "*.*")))
    if RAMfileName:
        RAMfile = open(RAMfileName, 'wb')
        Address = 0
...
def main_Catskull_write():
    if main_LoadROM() == 1:
        ...
        for ROMAddress in range(0x0000, 0x8000, 1):
            ...
            dev.write(0x01, [0x0A, 0x01, 0x04, 0x55, 0x55, 0xAA, 0x2A, 0xAA, 0x55, 0x55, 0x55, 0xA0, AddHi, AddLo,
                             Data1Byte[0]])
            ROMpos += 1
        messagebox.showinfo('Operation Complete', 'Writing Complete.')
    return
...
if dev is None:
    root.update()
    messagebox.showinfo("USB Error",
                        "I Cant find your hardware! Check the device is plugged in and the USB driver is installed")
    exit()
if dev is not None:
    root.update()
    dev.set_configuration()
    messagebox.showinfo("Welcome",
                        "Gen3 is a work in progress, please report any bugs or requests to Bennvenn@hotmail.com")
    main_CheckVersion()
